...
import numpy as np # Import numpy for explicit type casting
...
class SnowpackProfile:
    ...
    def __init__(self, filename: str, _load_data: bool = True):
        ...
        Args:
            ...
            _load_data (bool, optional): If False, initializes an empty object
                without reading the file. Used internally by the `slice` method.
                Defaults to True.
        """
    ...
    def _create_dataset_from_profiles(self, 
                                      profiles: List[Dict]):
        """
    ...
    def slice(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> 'SnowpackProfile':
        ...
        Args:
            start_date (Optional[str]): The start date for the slice (e.g., 'YYYY-MM-DD').
            end_date (Optional[str]): The end date for the slice (e.g., 'YYYY-MM-DD').

        Returns:
            SnowpackProfile: A new SnowpackProfile instance with the sliced data.
        """
        if self.data is None:
            logger.warning("Cannot slice an empty SnowpackProfile object.")
            return self

        try:
            time_slice = slice(start_date, end_date)
            sliced_data = self.data.sel(timestamp=time_slice)
        except Exception as e:
            logger.error(f"Failed to slice data for the given date range: {e}")
            # Return a new empty profile object
            new_profile = SnowpackProfile(self.filename, _load_data=False)
            new_profile.metadata = self.metadata
            return new_profile

        # Create a new instance and populate it with the sliced data
        new_profile = SnowpackProfile(self.filename, _load_data=False)
    ...
    def profile_to_dataframe(self, timestamp: str
